...
describe('Authorization', () => {
  describe('authorization method', () => {
    it('it should call next with valid auth token', async () => {
      const spy = sinon.spy();
      const testToken = JWT.sign({ test: '123' }, process.env.JWT_SECRET);
      await authorization({ headers: { authorization: testToken } }, null, spy);
      expect(spy.callCount).to.be.equal(1);
    });
    it('it should should throw when called with wrong token', () => {
      const spy = sinon.spy();
      return expect(authorization({ headers: { authorization: 'asfasf' } }, null, spy)).to.be.rejected;
    });
  });
});
